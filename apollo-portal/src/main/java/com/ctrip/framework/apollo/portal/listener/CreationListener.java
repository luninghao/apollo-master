...
/**
 * 对象创建监听器，目前监听AppCreationEvent 和 AppNamespaceCreationEvent 事件。
 */
@Component
public class CreationListener {
  ...
  @EventListener    // @EventListener 注解 + 方法参数，表示 #onAppCreationEvent(...) 方法，监听 AppCreationEvent 事件。
  public void onAppCreationEvent(AppCreationEvent event) {
    // 将 App 转成 AppDTO 对象
    AppDTO appDTO = BeanUtils.transform(AppDTO.class, event.getApp());
    // 获得有效的 Env 数组，例如 PROD UAT 等。
    List<Env> envs = portalSettings.getActiveEnvs();
    // 循环 Env 数组，调用对应的 Admin Service 的 API ，创建 App 对象，从而同步App到ConfigDB。
    for (Env env : envs) {
      try {
